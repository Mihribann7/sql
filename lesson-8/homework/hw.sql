...
select 
min(StepNumber) as minim,
max(StepNumber) as maxim,
status,
...
FROM (
  SELECT 
    DISTINCT YEAR(HIRE_DATE) AS Year,
    ROW_NUMBER() OVER (ORDER BY YEAR(HIRE_DATE)) -
    DENSE_RANK() OVER (ORDER BY YEAR(HIRE_DATE)) AS grp
  FROM EMPLOYEES_N
) AS Sub
GROUP BY grp
ORDER BY StartYear;
